...
# --- Game State ---
var money: float = 0.0
var current_tool_id: String = "spoon"

var tools_owned: Dictionary = {
	"spoon": {"owned": true, "level": 0},
...
func get_stat_value(stat_id: String) -> float:
	...
	return defn["base_value"] + defn["per_level"] * stat_levels[stat_id]
...
func get_stat_upgrade_cost(stat_id: String) -> float:
	var base_cost: float = stat_definitions[stat_id]["base_cost"]
...
func drain_water(swamp_index: int, gallons: float) -> void:
	...
	if actual > 0.0:
		var mpg: float = swamp_definitions[swamp_index]["money_per_gallon"]
